...
type discordEvent =
  ...
  | VoiceStateUpdate(response)
  | VoiceServerUpdate(response)
  | HeartbeatACK
  | Unknown;
...
let parseEvent = response => {
  ...
  | (0, Some(t)) =>
    ...
    | "VOICE_STATE_UPDATE" => VoiceStateUpdate(response)
    | "VOICE_SERVER_UPDATE" => VoiceServerUpdate(response)
    | _ =>
...
let parseVoice = message => message |> extractPayload |> parseEvent;
